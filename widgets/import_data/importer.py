...
class Importer(object):
    ...
    def _commitImport(self):
        ...
        importlog_layer.addFeatures([dst_feature])
    ...
    def _exportErrorsToCsv(self):
        ...
        try:
            ...
            # Header
            csvfile.write(';'.join(['Layer name', 'Feature ID', 'Message', 'Centroid X', 'Centroid Y']).encode('UTF-8'))
            csvfile.write('\n'.encode('UTF-8'))
    ...
    def _importLayer(self, src_layer, dst_layer, mapping, progressbar = None):
        ...
        for src_feature in src_dp.getFeatures(feature_request):
            if dst_layer.wkbType() == QgsWkbTypes.NoGeometry:
        ...
        for src_feature in source_features:
            ...
            for src_index, dst_index, constant_value, enabled, value_map in mapping.fieldMappings():
                ...
                elif dst_feature.fields()[dst_index].type() == QVariant.String and not isinstance(value, str):
            ...
            if dst_layer.wkbType() != QgsWkbTypes.NoGeometry:
                ...
                if dst_layer.wkbType() == QgsWkbTypes.Polygon and src_layer.wkbType() == QgsWkbTypes.LineString:
    ...
    def _getFieldsMappingTableItemWidget(self, layer, fieldname, value, secondary_value = None):
        ...
        if layer.editorWidgetSetup(fieldIndex).type() == 'ValueMap':
            config = layer.editorWidgetSetup(fieldIndex).config()
            if "map" in config:
                config = dict((v, k) for k, v in config["map"].items())
            ordered_config = OrderedDict(sorted(list(config.items()), key=lambda t: t[1]))
            return self._getCombobox(ordered_config, value, secondary_value)

        # Field editor is range
        elif layer.editorWidgetSetup(fieldIndex).type() == 'PreciseRange':
            config = layer.editorWidgetSetup(fieldIndex).config()
            if "map" in config:
                return self._getSpinbox(config["map"]['Min'], config["map"]['Max'], config["map"]['Step'], config["map"]['AllowNull'], value)
            else:
    ...
    def asIndexFieldMappings(self, destination_fields, source_fields=None):
        ...
        for source, destination, constant_value, enabled, value_map in self.fieldMappings():
            mapping.addFieldMapping(
                source_fields.indexFromName(source) if source is not None else source,
                destination_fields.indexFromName(destination),
                constant_value,
                enabled,
                value_map
            )
