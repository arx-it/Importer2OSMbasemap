...
class Project(QObject):
    ...
    def _addOpenStreetBasemap(self):
        ortho_url = 'type=xyz&url=https://tile.openstreetmap.org/%7Bz%7D/%7Bx%7D/%7By%7D.png&zmax=19&zmin=0'
        ortho_found = False
        for k, v in list(QgsProject.instance().mapLayers().items()):
            if v.source() == ortho_url:
                ortho_found = True
        ...
        if not ortho_found:
            ortho_layer = QgsRasterLayer(ortho_url, 'OpenStreetMap', 'wms')
            QgsProject.instance().addMapLayer(ortho_layer, False)
            QgsProject.instance().layerTreeRoot().addLayer(ortho_layer)
            main.qgis_interface.mapCanvas().setExtent(QgsRectangle(-682171,5225596,1061986,6759818))
